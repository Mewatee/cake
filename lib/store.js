/* ═══════════════════════════════════════════════════════════
   Order storage — shared by the Node server (Render/local) and
   the Vercel serverless functions.
   • If SUPABASE_URL + SUPABASE_KEY are set → Supabase (Postgres) via REST.
   • Otherwise → a local JSON file (works locally/Render; NOT on Vercel,
     whose filesystem is read-only — use Supabase there).
   ═══════════════════════════════════════════════════════════ */
const fs   = require('fs');
...
const FILE = process.env.ORDERS_FILE || path.join(__dirname, '..', 'server', 'orders.json');
